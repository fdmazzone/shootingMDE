...
\end{equation}
where $f:[0,T]\times\rr^n\to\rr^{n}$ is a vector field, and
...
\begin{obs}\label{rem:equi} We can allow $f(t,x).\vec{n}=0$ in \ref{eq:tran}, if we assume that in that case $x$  is an equilibrium point for $f$, that is $f(s,x)=0$, for every $s\geq t$.
\end{obs}
...
Now,  Theorem \ref{th:final} follows immediately.
 ...
 \section{An Example: An Impulsively Forced and Damped Pendulum }
 
 
    In order to illustrate the application of our main theorem, we will present a concrete example. We consider the  following periodic boundary value problem 
     
     \begin{equation} \label{eq:problema A}
	\left\lbrace \begin{array}{l}
		d\theta'= (-\sin \theta-\theta') \, d\lambda+g(t,\theta,\theta')\,d\mu,    \\
		\theta(0)-\theta(4\pi)=\theta'(0)-\theta'(4\pi)=0.
	\end{array}\right. \label{eq:example 2.1}
\end{equation}

The equation correspond to   a damped and impulsively forced pendulum. We take the viscosity constant to be equal to 1.0. We assume that the external force is impulsive and is given by  a sum of delta measures, specifically $\mu=\sum_{i=1}^{10}\delta_i.$ The function $g$ will be supposed sufficiently regular function; we will specify it shortly. Let us remember that $\lambda$ is the Lebesgue measure.
...
$$F(t,x)=\carac{[0,T]\setminus D}(t)
 ...
 +\carac{D}(t)
    ...
    \end{pmatrix}
 .$$
Here $\carac{A}$ denotes the indicator function with respect to the set $A$ and   $D$ is the set of discontinuity points of measure $\nu$,  concretely $D=\{1,\ldots,10\}$. 

Obviously, we can replace the ball in Theorem \ref{th:final} by any set that satisfies Brouwer's Theorem, in particular by a compact and convex set $K$. Hypothesis \ref{eq:tran} must be replaced by the condition 

\begin{equation}\label{eq:normal_ext}
 F(t,x)\cdot \vec{n}<0,\quad \text{for every $(t, x)$ with  } x\in\partial K \text{ and } \nu(\{t\})=0,
\end{equation}
where $\vec{n}$ denotes a normal exterior  vector with respect to $\partial K$. 

We will take the set $K$ as 

$$K=\left\{x\mid  -\pi<x_1<\pi,\; \frac{x_2^2}{2}-\cos x_1\leq 1\right\},$$
 In physical terms, $K$ is the set of states with energy less or equal than 1. We note that we can choose $\vec{n}=(\sin x_1,x_2)$. 
If $\nu(\{t\})=0$ then $t\notin D$. In this situation it is easy to see that inequality \eqref{eq:normal_ext} holds when $x\neq (\pm\pi,0)$. In the exceptional case that $x=(\pi,0)$, we apply Remark \ref{rem:equi}.





In order to  $F$ to satisfy \ref{eq:tele} we have to choose $g$ appropriately. For example, if $g$ satisfies that $|g(t,x)|\leq \frac{d(x,\partial K)}{M}$, where $M=\displaystyle\sup_{t\in D}\left\{\nu(\{t\})\right\}$,  then \ref{eq:tele} holds.  


Under all of the above assumptions, problem \eqref{eq:problema A} has a solution.

The shooting method can be used as an algorithm to numerically solve  periodic problems associated to MDEs. We programmed this algorithm in Python (it can be found at xxx). It numerically calculates Lebesgue-Stieltjes integrals and solves MDEs using Picard's successive approximation method. The convergence of this last procedure was established in \cite{P.Mazzone}. This is used to obtain the Poincaré mapping $P$.  Finally, the fixed point problem  $P(x)=x$ is solved using functions of the modulus \texttt{scipy.optimize}.  



Returning to the particular problem \eqref{eq:problema A},  we take  
$$g(x)=\max\left \{1-x_1^2-x_2^2,0\right\},$$
and we obtain that  the initial conditions $\theta(0)=-0.1019$ and $\theta'(0)=-0.1204$ correspond to a solution of  \eqref{eq:problema A}. Figure \ref{fig:sol} shows the graph of $\theta$ and $\theta'$  in the interval $[0,8\pi]$.  We also plot the contour lines of $h(x)=\|x-P(x)\|^2$ (see figure \ref{fig:mapa}). This graph shows us that it is plausible that the solution to the periodic problem found is unique.  
...
\begin{figure}[h]
 \centering
 \includegraphics[scale=.3]{Experimento_numerico/solucion.png}
 % solucion.png: 640x480 px, 100dpi, 16.26x12.19 cm, bb=0 0 461 346
 \caption{Graph solution in in the interval $[0,8\pi]$ of \eqref{eq:problema A}, $\theta$ (blue) and $\theta'$ (orange)}
 \label{fig:sol}\label{fig:sol}
\end{figure}

\begin{figure}[h]
 \centering
 \includegraphics[scale=.45]{Experimento_numerico/CurvasNivel.png}
 % CurvasNivel.png: 640x480 px, 100dpi, 16.26x12.19 cm, bb=0 0 461 346
 \caption{Level sets of $h(x)=\|x-P(x)\|^2$. The dot marked correspond to the (presumably unique) solution.}
 \label{fig:mapa}
\end{figure}
