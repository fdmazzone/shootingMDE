...
\end{abstract}
    ...
    \markboth { \tiny{Periodic Solutions to MDE through  Shooting Method}}
       ...
       \noindent{2020 {\it Mathematical Subject Classification.}}\\
        Primary 34B37. Secondary  34A36, 34A37 and  34A38.
...
The problem of finding periodic solutions to impulsive systems has attracted the attention of researchers. This problem has been addressed in the literature using various techniques; see, for example, \cite{Zhang-2010,Li-2014,Tian-2008,daoudi2013variational,li2020infinitely,Jingl-2013,zhang2008periodic,yang2011periodic,graef2015nontrivial,Liu-2019,heidarkhani2015infinitely}. To the best of our knowledge, this article is the first to focus on the shooting method. \comment{Revisar esto, lo acabo de agregar}
...
\item \label{pm2.5}$f$ is continuous at $t$, when  $\mu(\{t\})=0$.
 ...
 \item     There exists a ball $B(0, R)$ such that:
  ...
  \end{enumerate}
 	...
 	Then, the problem \eqref{eq:problema B} has at least a solution. 
\end{thm}
...
We briefly introduce some notations related to measure theory, we suggest  \cite{StanislawSaks,EdwinHewitt,folland, Evans, Carter} as  references to the theory introduced in this section.
...
$BV([0,T],\rr^n)$. If $\varphi\in BV([0,T],\rr^n)$, we denote by $\V(\varphi,[0,T])$ the \emph{total variation} of the function $\varphi$. As is well known, functions in $ BV([0,T],\rr^n)$ have one-sided limits at every $t\in [0,T]$ and are continuous except for a set that is at most countable. We note that $\varphi$ is continuous,  there exists a unique Borel measure, which we denote  $\mu_{\varphi}$, such that  $\mu_{\varphi}([a,b))=\varphi(b^-)-\varphi(a^-)$ for every $a$ and $b$ in $[0,T]$.  This measure is called the \emph{Lebesgue-Stieltjes measure} (see \cite{folland} for details). We will usually assume that $\varphi$ is left-continuous on the left, in which case  $\mu_{\varphi}([a,b))=\varphi(b)-\varphi(a)$.
...
\begin{thm}\label{th:prop_max} Let $\mu$ be a finite measure,  $\Omega$ be an open set of $\rr\times \rr^n$ and  $(0,\alpha)\in\Omega$. Suppose that $f:\Omega\to\mathbb{R}^n$ is a locally Lipschitz function with respect to the second variable for $t\geq 0$ and  $\varphi$ is  a  solution of \eqref{eq:pvi}
defined on a maximal half-open interval $[0,t_1)$.  Then, one and only one of the following assertions holds true.
\begin{enumerate}
...
Then,  using the Integral Part Formula, see  \cite[Th. 14.1]{StanislawSaks}, and taking into account the continuity of $F\circ h$,
we have  
\begin{equation*}
\begin{split}
...
For any  Borel set  $A$, we define $\mu_j$ as the  measure induced by $\mu$ on $D$ or \emph{jump part of } $\mu$, i.e.
...
We define $w(t)=c+\int_{[0,t)}u(s)\;d\mu$. Fix an arbitrary  $t\in  [0,T)$. Since  $D$ is a countable set,  we can assume that $ D\cap[0,t)=\left\lbrace t_n\right\rbrace _{n=1}^\infty$. In addition, as
	...
	\end{equation}
by \eqref{eq:4.1} there exists $N\in\nn$ such that


\begin{equation}\label{eq:sum_disc}
\sum_{n=N+1}^{\infty}\mu(\{t_n\})\leq \delta.
\end{equation}
Let $\{s_1,\ldots,s_N\}$ be an increasing reordering of the set $D_\delta:=\{t_1,\ldots, t_N\}$. Additionally we put $s_{N+1}=t$.
