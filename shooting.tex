...
\paragraph{Experiment 1: over-damped pendulum}  We take $c=2.0$, $D=\{1,\ldots, 6\}$,
...
\paragraph{Experiment 1: undamped pendulum}  In this example, we take  $c=2.0$, $D=\{1,\ldots, 6\}$ (again) and
$\mu =\sum_{j=1}^6\delta_j$ . The function $g$ is the same that  the previous example. In this case, we are interested in seeing what a periodic solution looks like when the impulse forces always have the same direction.



\begin{figure}[h]
 \centering
 \includegraphics[scale=.3]{Experimento_numerico/mapa_no_amort.png}
