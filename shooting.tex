...
\begin{equation} \label{eq:problema A}
	...
	\end{array}\right. 
\end{equation}
where $\nu:\mathscr{B}([0,T])\to \rr^m$ is a vector measure (see \cite{distel}), with finite total variation, $\mathscr{B}([0,T])$ is the $\sigma$-algebra of all Borel measurable subsets of $[0,T]$  and $F:[0,T]\times \rr^n\to \rr^{n\times m}$ is a matrix valued Carathéodory function with respecto to $\nu$, that is $F(t,x)$ is Borel measurable as function of $t$  for every $x\in \mathbb{R}^n$, and  continuous as function of $x$ for $\mu$-a.e. $t\in [0,T]$.
...
\begin{defi}\label{mu_a}
	Let $\mu$ be a finite positive measure and let $D=\{\tau\in I \mid \mu(\{\tau\})>0\}$.\comment{El intervalo I no seria [0,T]
}
...
\todoLS[]{$-\pi\leq x_1\leq \pi$ para que sea cerrado?}
...
$\mu =\sum_{j=1}^6(-1)^j\delta_j$ and $g(t,x)=\max\{1.9-\|x\|,0\}$. Then $M=1$, and, it is not hard to see that, $g$ verifies the inequality $|g|\leq d(x,\partial K)$. In Figure \ref{fig:mapa} the density plot of of $h(x)=\|x-P(x)\|^2$ is drawn.
 ...
 \caption{Level sets of $h(x)=\|x-P(x)\|^2$, $c=2.0$, $\mu =\sum_{j=1}^6(-1)^j\delta_j$. }
 \label{fig:mapa}
\end{figure}
...
\begin{figure}[h!]
 \centering
 \includegraphics[max width=.8\textwidth]{Experimento_numerico/solucion_amort.png}
 % solucion.png: 640x480 px, 100dpi, 16.26x12.19 cm, bb=0 0 461 346
 \caption{Graph solution $(\theta,\theta')$  in in the interval $[0,4\pi]$ of \eqref{eq:problema A} with  $c=2.0$, $\mu =\sum_{j=1}^6(-1)^j\delta_j$.\todoLS[]{yo le veo más discontinuidades, 12 para ser exacto}}
...
\begin{figure}[h]
 ...
 % mapa_no_amort.png: 1440x792 px, 100dpi, 36.58x20.12 cm, bb=0 0 1037 570
  \caption{Level sets of $h(x)=\|x-P(x)\|^2$, $c=0.0$, $\mu =0.5\sum_{j=1}^6\delta_j$. }
 \label{fig:mapa}
...
\begin{figure}[h]
 ...
 \caption{Graph solution $(\theta,\theta')$  in in the interval $[0,4\pi]$ of \eqref{eq:problema A} with  $c=0.0$, $\mu =0.5\sum_{j=1}^6\delta_j$..}
