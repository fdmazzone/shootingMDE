...
In Figure \ref{fig:sol}, we plot the corresponding solution. As can be seen, in this case, \textcolor{blue}{the mass maintains negative angles for most of its motion.}
\todoSA[inline]{La IA sugiere la siguiente redacción: in this case, the mass spends most of its motion at negative angles.}
...
\textcolor{blue}{The map clearly shows the compact region $K$ and a solution within it. The unstable solutions $(\pi,0)$ and $(-\pi,0)$ are on the edge of $K$ where the pendulum behaves like a simple pendulum. As in the previous example, the stable equilibrium (0,0) of the simple pendulum is no longer an equilibrium of our equation. The stable equilibrium is observed at $P=(1,338 ; 0,578)$,  obtained through an optimization algorithm, and its graph is shown in Figure \ref{fig:sol_no_amort}.}
